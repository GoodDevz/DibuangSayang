...
class Transaksi extends CI_Controller {
	...
	{	
		...
		if($this->session->userdata('level') == 'Anggota'){
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `NIP`, 
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM tbl_pinjam WHERE NIP = ? AND status = 'Di Kembalikan' 
	...
	public function detailpinjam()
	{
		...
		if($this->session->userdata('level') == 'Anggota'){
			...
			if($count > 0)
			{
				...
				AND NIP =?", 
				array($id,$this->session->userdata('NIP')))->row();
			}else{
    ...
    {	
		...
		$user = $this->M_Admin->get_tableid_edit('tbl_login','NIP',$this->input->post('kode_anggota'));
		error_reporting(0);
		if($user->nama != null)
		{
